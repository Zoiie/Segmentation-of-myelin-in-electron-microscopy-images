...
def topo_module(lh,gt):
    topoloss_sum=[]
    # target = torch.zeros(448*448*4-lh.shape[1]*lh.shape[2]*lh.shape[3])
    # for i in range(0,len(lh)):
    #     # print("{0}topo_lh".format(i),lh[i])
    #     # print("{0}topo_gt".format(i),gt[i].shape)
    #     lh_fla=lh[i].flatten()
    #     gt_fla = gt[i].flatten()
    #     # print("lh_fla:",lh_fla)
    #     lh=torch.cat((lh_fla, target), 0)
    #     gt = torch.cat((gt_fla, target), 0)
    #     lh=lh.resize(448,448,4)
    #     gt=gt.resize(448, 448, 4)
    #     print("lh gt reshape:",lh.shape,gt.shape)
...
if __name__ == "__main__":
    ...
    for epoch in range(args['nb_epochs']):
        ...
        for cnt_batch, batch_data in enumerate(train_loader):
            ...
            # print("batch_data[1]:", batch_data[1])
            ...
            topo_flag=False
            if topo_flag==True:
                topoloss=topo_module(preds_torch,gts_torch)
                # display(preds_torch)

                # Compute the loss
                loss_base= loss_function(preds_torch, gts_torch)

                #
                print("topoloss:",topoloss)
                loss=loss_base+0.00005*topoloss
