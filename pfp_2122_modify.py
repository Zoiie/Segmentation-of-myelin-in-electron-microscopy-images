...
if __name__=="__main__":
    ...
    for epoch in range(args['nb_epochs']):
        ...
        for cnt_batch, batch_data in enumerate(train_loader):
            ...
            #Compute the loss
            flag=True
            if flag and epoch>=100:
                loss_base=loss_function(preds_torch, gts_torch)
                loss_topo=topo_module(preds_torch,gts_torch)
                print("loss_topo:",loss_topo,"loss_base:",loss_base)
                # loss=0.2*topo_module(preds_torch,gts_torch)+loss_function(preds_torch, gts_torch)
                # loss_topo= torch.from_numpy(loss_topo)
                loss=0.6*loss_base+0.4*loss_topo
                print("mix loss",loss)
            else:
